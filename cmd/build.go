...
func Build(args []string, stdout, stderr io.Writer) error {
	...
	fs.StringVar(&cfg.AccentColor, "accent-color", cfg.AccentColor, "Accent color: Tailwind name (e.g. 'sky', 'rose') or hex (e.g. '#0ea5e9')")
...
func printBuildUsage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  --accent-color <c>   Accent color: Tailwind name (sky, rose, ...) or hex (default: sky)")
