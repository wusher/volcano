...
`
    ...
    serverProcess = await startServer(testDir, PORT, ['--accent-color=#ff5500']);
  });
  ...
  test('custom accent color is applied to page', async ({ page }) => {
    ...
    const hasAccentColor = await page.evaluate(() => {
      ...
      return accentVar.includes('ff5500') ||
  ...
  test('active nav item uses accent color', async ({ page }) => {
    ...
    const activeLink = page.locator('.tree-nav a.active, .tree-nav a[aria-current]');
